...
export default function OrderDetail() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      Swal.fire("Error", error?.response?.data?.error || "Unable to cancel", "error");
    }
  };
      ...
      >
        ...
        {order.status === "rejected" ? (
          ...
          <div className="timeline">
            <TimelineStep active={currentIndex >= 0} icon={<FaClock />} title="Pending" />
            <TimelineStep active={currentIndex >= 1} icon={<FaCheckCircle />} title="Accepted" />
            <TimelineStep active={currentIndex >= 2} icon={<FaShoppingBag />} title="Preparing" />
            <TimelineStep active={currentIndex >= 3} icon={<FaTruck />} title="Ready" />
            <TimelineStep active={currentIndex >= 4} icon={<FaBoxOpen />} title="Collected" />
          </div>
        )}

        {["pending", "accepted"].includes(order.status) && (
          <button className="btn btn-danger mt-4" onClick={handleCancel}>
            Cancel Order
          </button>
        )}
        <button
